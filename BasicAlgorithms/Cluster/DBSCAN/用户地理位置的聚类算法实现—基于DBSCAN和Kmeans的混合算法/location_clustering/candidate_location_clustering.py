...
def clustering_by_dbscan():
    ...
    # using "precomputed" as recommended
...
def clustering_by_kmeans2():
    ...
    res, idx = kmeans2(numpy.array([xy[:, 0], xy[:, 1], z]), 3, iter=20, minit='points')
    for r in res:
    ...
    # plt.scatter(xy[:, 0], xy[:, 1], s=20 * z + 9, c=colors)
    # plt.show()
...
def clustering_by_dbscan_and_kmeans2():
    ...
    for k in results.keys():
        ...
        z = numpy.sin(xy[:, 1] - 0.2 * xy[:, 1])
        z = whiten(z)
        res, idx = kmeans2(numpy.array([xy[:, 0], xy[:, 1], z]), 1, iter=20, minit='points')
